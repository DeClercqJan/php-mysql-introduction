...
if (isset($_SESSION["status_login"]) && $_SESSION["status_login"] == "succesful login") {
    ...
    if (isset($_POST["submit_delete"]) && $_SESSION["status_login"] == "succesful login") {
        $db = openConnection();
        ...
        echo "Your profile has been succesfully deleted. Register at <a href='register.php'>register.php</a";
    } else {
        try {
...
?>
    ...
    <?php
        ...
        if ($id == $_SESSION["id"]) {
            echo "<form action='profile.php?user=$id' method='POST'><input type='submit' name='submit_delete' value='delete' id='submit_delete'></form>";
            // TO DO if time: add edit button
        }
    }
    ?>
